...
b.  Our outcome variable is `is_canceled`. Transform this variable to the appropriate data class so that 
...
## Part 2 - Predicting daily rates

## Question 4

The dataset also contains information about the average daily rate (`adr`) for each reservation.

a.  Create a scatterplot of `adr` (y-axis) versus `adults` (x-axis), colored by `hotel` type.
    Add a linear regression line to the plot with `geom_smooth(method = "lm")`.

b.  Fit a model predicting `adr` from `adults` and `hotel` type.
    This model should match your plot from part (a).
...
```{r}
library(tidyverse)
d <- read_csv("lab/data/hotels.csv")
d |>
  filter(adr < 1000 & adr > 0) |>
  ggplot(aes(x = adults, y = log(adr), color = hotel)) +
  geom_point() +
  geom_smooth(method = "lm")

d |>
  filter(adr < 1000 & adr > 0) |>
  ggplot(aes(x = adults, y = adr, color = hotel)) +
  geom_point() +
  geom_smooth(method = "lm")
```
