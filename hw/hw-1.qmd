...
b. In a single pipeline, identidy all rural non-metro counties that appear as outliers in the boxplot you created in part (a). Your result should be a data frame with columns `county`, `county_type`, and `median_hh_income`.  (Hint: Use `filter()` with two conditions connected by the `&` operator.)
